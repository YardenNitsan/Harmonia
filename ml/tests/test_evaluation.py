...
def test_evaluation_ignores_masked_labels_and_boundaries(tmp_path):
    ...
    targets = dict(
        root=np.array([0, 12, 12, 0]), triad=np.array([1, 0, 0, 1]),
        seventh=np.zeros(4, dtype=int), bass=np.array([0, 12, 12, 0]),
        extensions=np.zeros((4, 4), dtype=int), boundary_times=np.array([1.5]),
    )
    np.savez(tmp_path / "track.npz", features=features, times=times,
             mask=np.array([True, False, False, True]), **targets)
    predictions = tmp_path / "predictions"
    predictions.mkdir()
    np.savez(predictions / "track.npz", **{**targets, "root": np.array([0, 1, 1, 0])})
    manifest = tmp_path / "manifest.json"
    manifest.write_text(json.dumps({"records": [{
        "track_id": "track", "prepared_file": "track.npz", "split": "validation",
        "performer_id": "fixture", "style": "fixture", "duration_seconds": 4,
    }]}))
    report = evaluate(manifest, "validation", tmp_path / "report.json",
                      prediction_dir=predictions, method_name="fixture")
